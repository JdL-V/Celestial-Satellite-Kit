function [r,v,yrot] = plotorbit_rot(I0, solver, y0, tstart, tfinal, npoints, mu, flag, plot, varargin)
...
%     disp('pertubed');
    ...
    [~,Y]=solver(@(t,y) keplerian_orbit(t,y,mu,I0),tspan,y0,options);
%     disp('normal');
end
r = Y(:,1:3);
v = Y(:,4:6);
...
function dy=keplerian_orbit(t,y,mu,I0)
...
M = [0 sin(u(10)*t/(2*pi)) cos(u(10)*t/(2*pi))]'*1e-7;
...
function dy=pertubed_keplerian_orbit(t,y,mu,J_2,R_e,I0)
...
% T = ker2cap(r,v)
% theta = wrapTo2Pi(2*pi*t/T) % esto hay que ver si se puede mejorar para no arrastrar errores 
% j_aero = r/norm(r);
% k_aero = v/norm(v);
% i_aero = cross(j_aero, k_aero);
% k_aero va en la dirección de la fuerza aerodinámica

% La M se sacaría de la fuerza aerodinámica
