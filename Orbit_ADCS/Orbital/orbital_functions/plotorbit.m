...
function dy=pertubed_keplerian_orbit(~,y,mu,J_2,R_e)
...
dy=[y(4) % r_x
    y(5) % r_y
    y(6) % r_z
    -mu/r_s^3*y(1)*(1 + 3*A_J2*(1 - 5*(y(3)^2/r_s^2)))   % v_x
    -mu/r_s^3*y(2)*(1 + 3*A_J2*(1 - 5*(y(3)^2/r_s^2)))   % v_y
    -mu/r_s^3*y(3)*(1 + 3*A_J2*(3 - 5*(y(3)^2/r_s^2)))]; % v_z
end